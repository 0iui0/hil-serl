    ...
    def _teleop_loop(self):
        ...
        while self._teleop_running and self._running:
            ...
            try:
                ...
                # Deadman: hold right button to enable motion
                if not buttons[1]:
                    time.sleep(dt)
                    continue
                ...
                xyz_d, rot_d = map_spacemouse_to_delta(
                    action, self._teleop_action_scale, self._teleop_rot_scale)
                delta = np.array(xyz_d + rot_d)
